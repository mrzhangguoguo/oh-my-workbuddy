...
3. **Inventory fallback-like code before editing**
   ...
   - For broad, ambiguous, cross-layer, or architectural fallback-like code, invoke the `ralplan` skill (`skill: ralplan`) for consensus resolution before edits.
   - Recursion guard: when already inside ralplan, a coding run, team, or another workflow, do not spawn a nested ralplan; record the finding and attach it to the active plan handoff instead.
...
7. **Finish with an evidence-dense report** (see Output Format below).
